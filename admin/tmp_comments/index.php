...
<ul>
	...
	try {
		if (isset($_GET['del'])) {
			$model = $_GET['del'];
			$sql = "DELETE FROM tmp_comment WHERE id = ?";
			$stmt = $conn->prepare($sql);
			$stmt->bindParam(1, $model);
			$stmt->execute();
		}
		$sql = "SELECT id, name, email, comment, date FROM tmp_comment ORDER BY id DESC";
		$res = $conn->query($sql);
		while ($row = $res->fetch()) {
	?>
	<li id="<?php echo $row['id'] ?>" class="list-group-item li">
	<p class="model-link"><?php echo $row['name']; ?></p>
	...
	<a href="edit.php?id=<?php echo $row['id'] ?>"><img class="edit float-right" src="edit.png"></a>
	</li>
	<?php
		}$conn=null;
